...
int client_config_load(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
  ...
  if (cfg->username == NULL) {
    long uid = (lrand48()&0x0fffffffffffffffL) + 9223372000000000000L;
    cfg->username = (char*)malloc(24);
    sprintf(cfg->username, "%ld", uid);
  }

	if (cfg->password == NULL) {
	  cfg->password = strdup("codein.tv");
  }
    ...
    if(cfg->topic_count == 0){
      cfg->topic_count = 1;
      cfg->topics = (char**)malloc(sizeof(char *));
      cfg->topics[0] = (char*)malloc(50);
      sprintf(cfg->topics[0], "codein/live/%s", cfg->username); 
      if (cfg->topic == NULL) {
        cfg->topic = strdup("codein/live");
      }
		}
    if (cfg->topic == NULL) {
      cfg->topic = cfg->topics[0];
    }
    if(!cfg->quiet) {
      fprintf(stderr, "sub topic: %s, pub topic: %s\n", cfg->topics[0], cfg->topic);
    }
	}
...
int client_config_line_proc(struct mosq_config *cfg, int pub_or_sub, int argc, char *argv[])
{
	...
	for(i=1; i<argc; i++){
		...
		}else if(!strcmp(argv[i], "-b") || !strcmp(argv[i], "--pub-topic")){
			if(i==argc-1){
				fprintf(stderr, "Error: -b argument given but no topic specified.\n\n");
				return 1;
			}
			if(mosquitto_pub_topic_check(argv[i+1]) == MOSQ_ERR_INVAL){
			 fprintf(stderr, "Error: Invalid publish topic '%s', does it contain '+' or '#'?\n", argv[i+1]);
...
int client_id_generate(struct mosq_config *cfg, const char *id_base)
{
	...
	if(cfg->id_prefix){
		...
		hostname[255] = '\0';
		len = strlen(id_base) + 10 + strlen(hostname);
